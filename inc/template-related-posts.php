 ...
 * @return array
 */
function epagreenlite_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = epagreenlite_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = epagreenlite_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = epagreenlite_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? epagreenlite_post_thumbnail( 'epagreenlite-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? epagreenlite_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? epagreenlite_posted_on( array( 'echo' => false ) ) : '';
